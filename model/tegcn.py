...
class Model(nn.Module):
    ...
    def forward(self, x):
        # batch, xyz, time, join, human
        N, C, T, V, M = x.size()
        print("N, C, T, V, M:{} {} {} {} {}".format(N, C, T, V, M))
...
class Model2(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=2, graph=None, graph_args=dict(), in_channels=3,
        ...
        if 1:
            ...
            self.l4 = TCNC_GCN_unit(64, 64, A, adaptive=adaptive, attention=attention)
            self.l5 = TCNC_GCN_unit(64, 128, A, stride=2, adaptive=adaptive, attention=attention)
            self.l5_ = TCNC_GCN_unit(128, 256, A, stride=2, adaptive=adaptive, attention=attention)
    ...
    def forward(self, x):
        ...
        # batch*human, xyz, time, join (32*2, 3, 300, 17)
        #N,C,T,V
        x = self.l1(x)
        x = self.l2(x)
        x = self.l3(x)
